...
Such systems have limitations. They are expensive and often slow to develop. There is a lot of rework since each country develops its own earthquake detection system on its own. Further, the systems themselves often have a high false positive rate and are not very effective.

## OpenEEW solution
To address these shortcomings, an organization called OpenEEW (short for early earthquake warning) has made an open source solution. It detects earthquakes using cheap $50 IoT accelerometers. As of 2020, they have deployed several hundred of these across Latin America.

They have made the accelerometer data publically available: there is 1TB of it already on S3. This represents 3 years of sensor data, from 2017 to 2020 YTD
...
OpenEEW is collecting interesting and valuable data. However, at present there is no method for tagging and studying past earthquakes as detected by the system.

Studying past earthquakes is likely of interest for analysts and researchers. A data platform would store metadata about these earthquakes so they could be more easily studied.

# Demo
This demo shows how some historical earthquakes are captured by my solution. I am visualizing the data in Tableau.

Larger circles indicate earthquakes of higher intensity. Darker circles indicate earthquakes with longer durations.

The circles are at sensor locations in Mexico.

# Architecture
I deployed a Pulsar cluster using the "from bare metal" guide, available from Pulsar documentation. I hosted the cluster on several AWS EC2 instances.
...
(show pic of instances)

(show architecture pic)
...
OpenEEW has a library for downloading their data, but it not functioning properly at present. I opened a ticket to report this bug. I had to implement the data pulling on my own; import_data.py would have been much shorter if the OpenEEW library had been working.
...
## Pulsar functions
I spent a while trying to deploy functions in Pulsar, and was able to figure most of it out. However I have left the detection function in the consumer for now.
